...
- **搜索类型 + 自定义搜索引擎**：搜索框上方 tab 为搜索类型（网页 / 图片 / 视频 / 新闻 / 音乐），各引擎按类型提供地址；搜索引擎本身是可编辑数据——点搜索框左侧 Logo 快速切换，设置里可自由添加 / 编辑 / 删除引擎（地址用 `%s` 代表搜索词），输入网址则直接跳转。
...
- 类型（`TYPES`，`js/app.js`）：`网页 / 图片 / 视频 / 新闻 / 音乐`，顶部 tab 直接切换。
- 引擎（存于 `settings.engines`，随云端同步）：每条含名称和各类型地址模板，`%s` 为搜索词占位符，例如 `https://www.bing.com/images/search?q=%s`。某类型没填模板的引擎不会出现在该类型下；切换类型时若当前引擎不支持，会自动换到支持的引擎，并且每个类型记住各自上次用的引擎。
