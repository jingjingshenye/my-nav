...
- **搜索（对齐 inftab）**：搜索框上方 tab 为搜索类型 `html / photos / news / videos / map`；引擎自带各类型地址模板，搜索词直接拼接在地址末尾（或替换 `%s`）。内置引擎库 8 个（baidu、bing、yahoo、yandex、duckduckgo、360搜索、sogou、yandex ru），默认启用 baidu + bing；点搜索框左侧 Logo 切换引擎，下拉里的「＋ 添加」可从引擎库启用或自定义引擎。输入网址直接跳转；输入关键字实时显示搜索建议（可关）。
- **外观自定义**：网格行列数（默认 3×6）、隐藏图标名称、5 款内置壁纸 + 自定义壁纸 URL、图标服务地址可配、图标加载失败自动退回字母头像、鼠标悬停网格两侧显示翻页箭头。
